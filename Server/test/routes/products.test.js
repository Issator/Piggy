 ...
 */
 const createDummyProduct = async (productData, token) => {
    const response = await request(app).post('/product/add').send(productData).set('token', token)
    if(response.ok) {
...
describe("Test products", () => {
    ...
    describe("POST /products/add", () => {
        test("should create product", async () => {
            ...
            const response = await request(app).post("/products/add").send(req).set("token", token)
    ...
    describe("GET /product/:id", () => {
        test("id: 1-> should return product with id = 1", async () => {
            const response = await request(app).get("/product/1").set("token",token)
    ...
    describe("PUT /product/:id", () => {
        test("should update product", async () => {
            ...
            //send request
            const response = await request(app).put("/product/1").send(req).set("token", token)
            expect(response.statusCode).toBe(200)
            expect(response.body.id).toBeDefined()
            expect(response.body.name).toBe(req.name)
            expect(response.body.end_date).toBeDefined()
        ...
        // //TODO: Wait for savings to get user id or add user id to product
        // test.todo("should fail (Permission denied)", async () => {
        ...
        // //TODO: Wait for savings to get user id or add user id to product
        // test.todo("should update product (admin)", async () => {
        ...
        // //TODO: Wait for savings to get user id or add user id to product
        // test.todo("should fail (product dont exist)", async () => {
    ...
    describe("DEL /products/:id", () => {
        ...
        beforeEach(async () => {
            const product_id = await createDummyProduct(dummyProduct,token)
        ...
        test("Should fail (permission denied)", async () => {
            ...
            const response = await request(app).del("/product/" + dummy_id).set("token",token)
            expect(response.statusCode).toBe(406)
            expect(response.body.message).toBeDefined()
        })
        ...
        test("should delete product (delete by ower)", async () => {
            ...
            const response = await request(app).del("/product/" + dummy_id).send(req).set("token",dummy_token)
            expect(response.statusCode).toBe(200)
            expect(response.body.name).toBeDefined()
            expect(response.body.end_date).toBeDefined()
            expect(response.body.cost).toBeDefined()
        ...
        test("should delete user (admin permission)", async () => {
            ...
            const response = await request(app).del("/product/" + dummy_id).set("token",admin_token)
            expect(response.statusCode).toBe(200)
            expect(response.body.name).toBeDefined()
            expect(response.body.end_date).toBeDefined()
            expect(response.body.cost).toBeDefined()
        ...
        test("should fail (product dont exist)", async () => {
            ...
            const response = await request(app).del("/product/-1").set("token",admin_token)
            expect(response.statusCode).toBe(400)
            expect(response.body.message).toBeDefined()
        })
    })
